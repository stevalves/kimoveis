...
const createSheduleController = async (
  req: Request,
  res: Response
): Promise<Response> => {
  ...
  if (req.body.hour != `${hours}:${minutes}`)
  ...
  if (day.length != 2 || Number(day) > 30 || Number(day) < 0)
    throw new AppError("bed format B", 400);
  let month = date.split("/")[2];
  if (month.length != 2 || Number(month) > 12 || Number(month) < 0)
    throw new AppError("bed format C", 400);
  if (req.body.date != `${year}/${day}/${month}`)
    throw new AppError("bed format BODY", 400);
...
const readSchedulesByRealEstateIdController = async (
  req: Request,
  res: Response
): Promise<Response> => {
